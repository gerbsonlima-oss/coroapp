...
export class ChatCommandService {
  ...
  static async handleUserMessage(
    ...
    const action = normalizedText.startsWith("__action:") ? normalizedText.replace("__action:", "") : null;
    const intent = action
      ? action === "restart"
        ? "restart"
        : action === "cancel"
    ...
    } else if (!isAdmin && (state.step !== "idle" || action === "start_flow" || intent === "create_event" || intent === "create_song")) {
  ...
  private static async processStateStep(params: {
    ...
    const { state, tenantId, userId, text, files, intent, push } = params;
    ...
    switch (state.step) {
      case "awaiting_start_choice": {
        push(
          createBotMessage("Se quiser, eu já começo o fluxo completo para você. Toque em 'Criar evento + música'.", {
            quickReplies: [{ label: "Criar evento + música", action: "start_flow" }],
          })
        );
        return state;
      }
